        ...
        for e in frame_to_events.get(frame, []):
            ...
            if isinstance(e, sc2reader.events.UnitBornEvent) or isinstance(e, sc2reader.events.UnitDoneEvent):
                ...
                if unit.is_army or unit.is_worker or unit.is_building:
                    ...
                    assert name in fields, f"{name} {unit.is_army} {unit.is_worker} {unit.is_building}, {unit._type_class.name}"
...
if __name__ == "__main__":
    with Pool(20) as pool:
        series = pool.map(replay_to_series, [f for f in os.listdir("replays") if f.endswith("SC2Replay")])
