...
base_names = set(["Hatchery", "Lair", "Hive", "Nexus", "CommandCenter", "CommandCenterFlying", "OrbitalCommand",
...
BaseType = Enum("BaseType", "MAIN EXPANSION PROXY NONE")
...
class BaseTracker(object):
    ...
    def handleUnitInitEvent(self, event, replay):
        ...
        if self.resource_locs is None:
            self.resource_locs = [o.location for o in replay.objects.values() if
                                  o.name and ("Mineral" in o.name or "Vespene" in o.name) and hasattr(o, "location")]
            try:
                self.resource_clusters = AffinityPropagation(preference=-1000, random_state=None).fit(
                    self.resource_locs).cluster_centers_
            except:
        ...
        if event.unit.is_building:
            ...
            self.under_construction[event.unit.id] = (event.location, event.unit.owner.team_id, event.unit.finished_at,
                                                      is_mining_loc(self.resource_clusters,
                                                                    event.location) and event.unit.name in base_names)
    ...
    def handleUnitDiedEvent(self, event, replay):
        if event.unit.is_building and "Flying" not in get_current_name(event.unit, event.frame):
            if event.unit.id not in self.building_locs and event.unit.id not in self.under_construction:
                self.logger.info(
                    f"dying {event.unit}, built {event.unit.finished_at} missing from dicts as of {event.frame}, probably due to missed landing: {replay.filename}")
    ...
    def handleTargetPointCommandEvent(self, event, replay):
        if event.ability_name.startswith("Land"):
            ...
            if len(targets) == 0:
                self.logger.error(
                    f"Landing {target_type} but none selected. event={event}, selects={selects}, names={[get_current_name(s, event.frame) for s in selects]}")
            else:
                ...
                if unit.id in self.building_locs:
                    ...
                    for e in [e for e in replay.events[::-1] if e.frame < event.frame]:
                        if isinstance(e,
                                      sc2reader.events.UnitTypeChangeEvent) and e.unit == unit and e.unit_type_name == f"{target_type}Flying":
                ...
                self.building_locs[unit.id] = (event.location[:2], unit.owner.team_id, unit.finished_at,
                                               is_mining_loc(self.resource_locs,
                                                             event.location[:2]) and unit.name in base_names)
                self.lookup[event.frame] = self.building_locs.copy()
                self.keyframes.append(event.frame)
    ...
    def handleBasicCommandEvent(self, event, replay):
        if event.ability_name.startswith("Lift"):
            ...
            if len(targets) == 0:
                self.logger.error(
                    f"Lifting {target_type}, but none selected. event={event}, selects={selects}, names={[get_current_name(s, event.frame - 1) for s in selects]}")
            else:
                ...
                for unit in targets:
                    if unit.id not in self.building_locs:
                        ...
                        for e in replay.events[::-1]:
                            if unit.finished_at is not None and unit.finished_at < e.frame < event.frame and isinstance(
                                    e,
                                    sc2reader.events.UnitTypeChangeEvent) and e.unit == unit and e.unit_type_name == target_type:
                                # most recent landing
                                self.logger.info(
                                    f"found recent landing {e}, last known loc = {get_last_known_loc(unit.id, e.frame, self.lookup)}, built at {self.lookup[unit.finished_at][unit.id]}")
    ...
    def handleEndGame(self, event, replay):
        try:
            ...
            for frame in self.keyframes:
                ...
                while -1 in af.labels_ and count < 100:  # indicates clustering did not converge, so we retry until it does (giving up after 100 tries)
                    af = AffinityPropagation(preference=[0 if p else -5000 for p in prefs], random_state=None,
                ...
                for k in range(n_clusters):
                    ...
                    mining_locs = [(loc, finish) for loc, finish, pref in
                                   zip(locs[labels == k], finishes[labels == k], prefs[labels == k]) if pref]
                    if len(mining_locs) > 1:
                        ...
                        for i, (loc, finish) in enumerate(to_split):
                            ...
                            members = [(loc, finish) for loc, finish, pref in
                                       zip(locs[labels == k], finishes[labels == k], prefs[labels == k]) if not pref]
                            for ml, mf in members:
                                if dist(ml, loc) == min(dist(ml, x[0]) for x in
                                                        [original] + to_split[:i] + to_split[i + 1:]) and mf >= finish:
                                    self.logger.debug(
                ...
                new_centers = []
                for loc in locs:
                    if all(dist(loc, c) / self.map_dim > 0.1 for c in
                           centers):  # too far away from any cluster center, should be split
                        if any(dist(loc, select_center(cs)) / self.map_dim <= 0.1 for cs in
                               new_centers):  # close to an already split building, merge
                ...
                for cs in new_centers:
                    ...
                    cluster_centers_indices = np.append(cluster_centers_indices,
                ...
                for unit_id, loc, team_id, label in zip(unit_ids, locs, teamids, labels):
                    ...
                    pdict[team_id].base_cluster[frame][unit_id] = BaseCluster(label, Point2(
                        locs[cluster_centers_indices[label]]), base_types.get(label, BaseType.PROXY), team_id)
        ...
        for player in replay.players:
            ...
            assert len(
                player.bases) == replay.frames + 1, f"{len(player.bases)} base entries, {replay.frames} frames {sorted(player.bases.keys())}"
