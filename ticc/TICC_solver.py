...
class TICC:
    def __init__(self, window_size=10, number_of_clusters=5, lambda_parameter=11e-2,
        ...
        self.pool = Pool(processes=self.num_proc)  # multi-threading
    ...
    def fit(self, input_series):
        ...
        times_series_arr = input_series.copy()
        time_series_rows_size, time_series_col_size = input_series.shape
        ...
        for iters in range(self.maxIters):
            ...
            if iters == 0 and any(v == 0 for v in len_train_clusters.values()):
                raise ValueError("At least one cluster has zero points on first iteration", len_train_clusters)

            # train_clusters holds the indices in complete_D_train
            # for each of the clusters
            opt_res = self.train_clusters(cluster_mean_info, cluster_mean_stacked_info, complete_D_train,
                                          empirical_covariances, len_train_clusters, time_series_col_size, self.pool,
                                          train_clusters_arr)
        ...
        for cluster in range(self.number_of_clusters):
            ...
            if cluster_length != 0:
                ...
                if self.window_size == 1:
                    S = np.cov(np.transpose(D_train), ddof=0)
                else:
